...
作为知识维护总控智能体，通过 ActionDock 纯动作规范依托特权维护服务驱动知识中枢的端到端自动化维护闭环。负责多代码仓与单代码仓的分支自动同步、知识冲突语义消解、代码变更扫描判定、子智能体派发与边界约束、断链与状态差异审查门禁、统一发布推送以及检查点水位推进。
...
主控智能体与子智能体保持清晰的职责分工与权限边界：

- **主智能体核心职责**：
  - 负责维护流程全局编排，统一持有与调用特权维护服务（`--profile skm`）。
  - 执行代码分支同步与冲突消解闭环。
  - 执行变更扫描并作出场景判定决策。
  - 按照模块与改动范围对子智能体进行任务切片与并发调度。
  - 执行全库断链物理门禁校验与工作区状态审查。
  - 统一执行文档发布推送。
  - 推进检查点水位并输出结算报告。
- **子智能体职责与约束**：
  - 必须挂载并遵循 [项目知识库维护技能](../project-knowledge-maintainer/SKILL.md) 规范执行。
  - 专注于分配范围内的具体源码研读与知识文档编写。
  - 受控使用 `workspace/files.edit` 提交精准局部补丁，或使用 `workspace/files.write` 产出新流程文档。
  - 严禁越权执行分支同步、跨仓变更、文档发布或检查点推进等总控操作。
  - 完成任务后向主智能体输出结构化核验结论与改动清单。
